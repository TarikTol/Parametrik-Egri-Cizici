 ...
 *   Grafik Çizim Kütüphanesi | Sürüm 2.0 
 *   o.arik@edu.etu.tr
 */
...
public:
    ...
    void animatePoint(double px[], double py[], int size, int time=100, std::string color="black", double radius=1);  
    template <class vector> void drawVector(vector V, std::string color = "black", int radius = 1);
    template <class vector> void drawVectors(vector V[], int size, std::string color = "black", int radius = 1);
...
void canvas::animatePoint(double px[], double py[], int size, int time, std::string color, double radius)
{
    openFile();
    ...
    {
        outputFile << mapX(px[i]) << ", ";
    }
    outputFile << mapX(px[size-1]) << "];\n";

    outputFile << "const py = [";
    for(int i=0; i<size-1; i++)
    {
        outputFile << mapX(py[i]) << ", ";
    }
    outputFile << mapX(py[size-1]) << "];\n";

    outputFile << "ind = 0;\n";
    outputFile << "size = " << size << ";\n";

    outputFile << "function draw() {\n";
    outputFile << "ctx.clearRect(0, 0, " << Width << ", " << Height << ");\n";
    outputFile << "ctx.beginPath();\n";
    outputFile << "ctx.moveTo(px[ind], py[ind]);\n";
    outputFile << "ctx.arc(px[ind], py[ind], " << radius << ", 0, 2 * Math.PI);\n";
    outputFile << "ctx.strokeStyle = \"" << color << "\";\n";
    outputFile << "ctx.stroke();\n";
    outputFile << "ind++;\n";
    outputFile << "if(ind==size){ind=0;}\n";
    outputFile << "}\n";

    outputFile << "window.setInterval(draw," << time << ");\n";
...
void canvas::drawVector(vector V, std::string color, int radius)
{
    drawPoint(V.getX(), V.getY(), color, radius);
}

template <class vector>
void canvas::drawVectors(vector V[], int size, std::string color, int radius)
{
    for(int i=0; i<size; i++)
        drawPoint(V[i].getX(), V[i].getY(), color, radius);
}
...
void canvas::startDoc()
{
    ...
    outputFile << "<canvas id=\"myCanvas\" width=\"" << Width <<"px\" height=\"" << Height << "px\" style=\"border:1px solid #000000;\">\n";
